...
export default async (subject: string) => {               
                ...
                for (let i =0; i<25; i++) {                                   
                    await delay(5000);
                    emailResponse = await outlookRestHelper.getOutlookVerificationEmail(accessToken, subject);
                    if(! emailResponse.data.value[0]){
                        console.log(subject + " - request attempt " + i);
                        continue;                        
                    }
                    else {
                    console.log(subject + " - Email found");    
                    break;
                    }
                }
                ...
                const verifyUrlRegex = /(https?:\/\/)?([\da-z\.-=%-]+)\.([a-z]{2,6}(?=.*user))([\/\w\.-=%-]*)*\/?/g
